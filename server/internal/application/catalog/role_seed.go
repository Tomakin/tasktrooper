...
func productManagerAgent() roleAgentDef {
	return roleAgentDef{
		...
		rules: []domain.CreateOrchestratorRuleRequest{
			...
			rule("backlog-quality", 90, "Every task created must contain a user story, measurable acceptance criteria as observable scenarios (precondition, action, expected result) written entirely in the board's language, and an out-of-scope section. Ambiguous requests get a clarification question before implementation tasks are opened."),
