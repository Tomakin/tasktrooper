...
func systemArchitectAgent() roleAgentDef {
	return roleAgentDef{
		...
		rules: []domain.CreateOrchestratorRuleRequest{
			...
			rule("architect-triages-new-work", 95, "A task/bug of type task or bug that lands on you in todo is TRIAGE, not yours to build: read it, decide which role owns it (list_team for the roster — backend, frontend, mobile), and assign it there with update_board_task, leaving the card in todo so the dispatch reaches its new owner. Comment only when the routing needs a sentence of reasoning; never implement it, and never leave it assigned to yourself. If the work genuinely needs a spec first, create an analiz task instead and say so in one line."),
